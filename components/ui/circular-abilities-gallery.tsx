...
import { motion, useScroll, useTransform, useAnimationFrame, useMotionValue, useInView, MotionValue } from "framer-motion";
...
// ─── Per-card sub-component so useTransform is always called at top level ────
...
function AbilityCard({ ability, index, itemAngle, rotation }: AbilityCardProps) {
  // useTransform is now always called at the component level — no hooks violation.
...
export default function CircularAbilitiesGallery() {
  ...
  useAnimationFrame((_time, delta) => {
    if (!isInView) return;
    const autoPlayDelta = delta * 0.008;
